...
__kernel void OptFlow_C1_D0(__constant unsigned char* input_1, __constant unsigned char* input_2, int imgSrcWidth, int imgSrcOffset, int imgDstWidth,
                            int imgDstOffset, __global signed char* output_X, __global signed char* output_Y, int blockSize, int blockStep, int scanRadius) {
      ...
      if ((currXshift < ScanDiameter) && (currYshift < ScanDiameter)) {
        ...
        for (int i = 0; i < blockSize; i++) {
          for (int j = 0; j < blockSize; j++) {
            atomic_add(&(abssum[currYshift][currXshift]), abs(input_1[((imgSrcOffset + blockX * (blockSize + blockStep)) + scanRadius + i) +
  ...
  if ((threadY == 0) && (threadX == 0)) {
    ...
    if ((abssum[scanRadius][scanRadius] - minvalFin) <=
        MinValThreshold)  // if the difference is small, then it is considered to be noise in a uniformly colored area
    {
...
__kernel void Histogram_C1_D0(__constant signed char* inputX, __global signed char* inputY, int width, int offset, int scanRadius, int ScanDiameter,
                              __global signed char* valueX, __global signed char* valueY, int TestDepth, __global signed char* outVecX,
