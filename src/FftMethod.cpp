...
FftMethod::FftMethod(int i_frameSize,
                     ...
                     bool i_storeVideo,
                     bool i_raw_enable,
                     bool i_rot_corr_enable, bool i_tilt_corr_enable,
    ...
    for(int i = 0;i< sqNum;i++){
        for(int j = 0;j<sqNum;j++){
            ...
            if(raw_enable){
                ...
                if(pow(shift.x,2)+pow(shift.y,2) > max_px_speed_sq || absd(shift.x) > ((double)samplePointSize/2) || absd(shift.y) > ((double)samplePointSize/2)){
