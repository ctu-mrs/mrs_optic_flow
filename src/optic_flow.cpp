...
public:
  ...
  OpticFlow(ros::NodeHandle& node) {
    ...
    VelocityPublisher   = private_node_handle.advertise<geometry_msgs::TwistStamped>("velocity", 1);
    VelocitySDPublisher = private_node_handle.advertise<geometry_msgs::Vector3>("velocity_stddev", 1);
    if (raw_enable) {
      VelocityRawPublisher = private_node_handle.advertise<geometry_msgs::TwistStamped>("velocity_raw", 1);
    }
